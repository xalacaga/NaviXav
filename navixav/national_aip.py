    ...
    def handle_endtag(self, tag: str) -> None:
        ...
        if tag == "tr" and self._rows:
            ...
            for href, title in row["links"]:
                self.links.append((href, title or row_title))
...
def national_chart_category(value: str) -> str:
    ...
    if "_IAC_" in f"_{name}_":
        return "Approches IAC"
    if "_SID_" in f"_{name}_":
        return "Départs SID"
    if "_STAR_" in f"_{name}_" or "_TRAN_" in f"_{name}_":
        return "Arrivées STAR"
    if any(token in f"_{name}_" for token in ("_ADC_", "_APDC_", "_GMC_", "_PDC_")):
        return "Aérodrome et roulage"
    if any(token in f"_{name}_" for token in ("_AOC_", "_PATC_")):
        return "Obstacles"
    if "_VAC_" in f"_{name}_" or "_VFR_" in f"_{name}_":
